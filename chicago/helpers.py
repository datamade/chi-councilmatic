def topic_classifier(title):
    ...
    if any(
        s in title
        for s in ("issuance of permits for sign(s)/signboard(s)", "install signs")
    ) or title.startswith("sign(s)"):
    ...
    if title.startswith(("conduct of sidewalk sale", "sidewalk sale")):
    ...
    if "transfer of tif funds" in title or (
        "allocation of" in title and "tif funds" in title
    ):
        return tags + ["Non-Routine", "Tax Increment Financing"]

    if title.startswith("tif ") or title.endswith("tif") or " tif " in title:
        return tags + ["Non-Routine", "Tax Increment Financing"]
    ...
    if (
        "neighborhood improvement program" in title.lower()
    ...
    if any(
        s in title
        for s in ("s.s.a.", " ssa", "special service no", "special service area")
    ):
    ...
    if title.lower().startswith(
        ("issuance of permit", "issuance of tent permit", "issuance of license")
    ):
        return tags + ["Non-Routine", "Fee Waiver"]
    ...
    if title.startswith(("historical landmark fee", "landmark fee waiver")):
        return tags + ["Non-Routine", "Fee Waiver"]
    ...
    if title.startswith(
        ...
        return tags + [
            "Non-Routine",
            "Free Water and Sewer for Non Profit Organizations",
        ]
    ...
    if title.startswith(
        ("traffic sign", "traffic warning sign", "miscellaneous signs")
    ):
    ...
    if title.startswith(
        ("closed to", "close to", "closure to vehic", "traffic closure")
    ):
        return tags + ["Non-Routine", "Traffic Closure"]

    if title.startswith(("vehicle weigh", "weight limitation", "weigh limitation")):
    ...
    if title.startswith(
        (
            "one time exception to wrigley field",
    ...
    if title.startswith(("pay rate of hospital", "payment of hospital and med")):
        return tags + ["Routine", "Police and Firefighter Medical Bills"]

    if title.startswith(
        ("independent police review", "police review authority", "police board")
    ):
    ...
    # Pensions
    if title.startswith(("laborers' and retirement board", "retirement board")):
    ...
    if any(
        s in title
        for s in ("tax levy", "tax levied", "levy of real estate tax", "tax levies")
    ):
    ...
    if title.startswith(
        ...
        return tags + ["Non-Routine", "Getting and Giving Land"]
    ...
    if title.startswith(
        ("correction of city council journal", "correction of journal")
    ):
    ...
    if (
        title.startswith(("city council rule", "amendment of rule"))
        or "rules of order and procedure" in title
    ):
